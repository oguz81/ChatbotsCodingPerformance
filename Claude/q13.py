...
def milne_predictor(f, y_prev, t_prev, h):
    """
    Milne's predictor formula (4-step explicit method).
    ...
    return y_prev[3] + (4*h/3) * (2*f_n - f_n1 + 2*f_n2)

def milne_corrector(f, y_pred, t_next, y_prev, t_prev, h):
    """
    Milne's corrector formula (3-step implicit method).
    
    Args:
        f: Function that defines the system of ODEs
        y_pred: Predicted value of y at the next step
        t_next: Time at the next step
        y_prev: List of previous values [y_n, y_n-1, y_n-2]
        t_prev: List of previous times [t_n, t_n-1, t_n-2]
        h: Step size
    ...
    return y_prev[2] + (h/3) * (f_next + 4*f_n + f_n1)

def solve_ode_milne(f, y0, t0, tf, h):
    """
    Solve a system of ODEs using Milne's method.
    ...
    # Use RK4 for the first 4 steps to bootstrap Milne's method
    for i in range(min(4, n_steps)):
        y_values[i+1] = runge_kutta_4th_order(f, y_values[i], t_values[i], h)
        t_values[i+1] = t_values[i] + h
    
    # Use Milne's method for the remaining steps
    for i in range(3, n_steps):
        # Milne's predictor
        y_prev = [y_values[i], y_values[i-1], y_values[i-2], y_values[i-3]]
        t_prev = [t_values[i], t_values[i-1], t_values[i-2], t_values[i-3]]
        y_pred = milne_predictor(f, y_prev, t_prev, h)
        
        # Milne's corrector
        t_next = t_values[i] + h
        y_corr = milne_corrector(f, y_pred, t_next, [y_values[i], y_values[i-1], y_values[i-2]], 
                                [t_values[i], t_values[i-1], t_values[i-2]], h)
        
        # Apply modified corrector iteration for better stability
        # Typically, one iteration is sufficient
        y_final = milne_corrector(f, y_corr, t_next, [y_values[i], y_values[i-1], y_values[i-2]], 
                                 [t_values[i], t_values[i-1], t_values[i-2]], h)
        
        # Update for the next step
        t_values[i+1] = t_next
        y_values[i+1] = y_final
...
def main():
    ...
    t_values, y_values = solve_ode_milne(ode_system, y0, t0, tf, h)
    
    # Print results
    print("Solving y'' - 3y' + 2y = 0 with y(0) = -1, y'(0) = 0")
    print("Using Milne's Method with h =", h)
    print(f"\n{'t':^10}{'y':^15}{'y\'':^15}")
    print("-" * 40)
