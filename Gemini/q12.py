...
def solve_ode_abm(f, t_span, y0, h):
    """
    ...
    if n_steps >= 4:
        ...
        for i in range(3, n_steps - 1):
            ...
            y_predict = y_values[i] + h/24 * (55*f_n - 59*f_n_minus_1 + 37*f_n_minus_2 - 9*f_n_minus_3)

            # Corrector (Adams-Moulton 4th order)
            f_predict = np.array(f(t_values[i+1], y_predict))

            y_values[i+1] = y_values[i] + h/24 * (9*f_predict + 19*f_n - 5*f_n_minus_1 + f_n_minus_2)
    elif n_steps > 1:
...
if __name__ == "__main__":
    ...
    # Solve the ODE using the fourth-order Adams-Bashforth-Moulton method
    t_values, solution = solve_ode_abm(f, t_span, y0, h)
